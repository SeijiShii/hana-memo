...
describe('mapStripeEvent', () => {
  it('WH01: ai_credits qty=1 → grantCredits 10 (revise_001)', () => {
  ...
  it('WH06: metadata.userId 欠落 → ignore', () => {
    expect(mapStripeEvent(event({ metadata: { type: 'ai_credits' } })).op).toBe('ignore');
  });
  it('未知の metadata.type → ignore', () => {
    expect(mapStripeEvent(event({ metadata: { userId: 'user_1', type: 'donation' } })).op).toBe(
      'ignore',
    );
  });

  it('quantity 欠落 → デフォルト 1 (credits 10)', () => {
    const op = mapStripeEvent(event({ metadata: { userId: 'user_1', type: 'ai_credits' } }));
    expect(op).toMatchObject({ op: 'grantCredits', credits: 10 });
  });
...
describe('applyBillingWebhook', () => {
  it('WH01: ai_credits → insertUnlock + grantCredits(10) + recordEvent', async () => {
  ...
  it('ignore event → unlock せず recordEvent のみ', async () => {
    const { store, spies } = makeStore();
    const res = await applyBillingWebhook(store, event({ type: 'payment_intent.created' }));
    expect(res.applied).toBe(false);
    expect(spies.insertUnlock).not.toHaveBeenCalled();
    expect(spies.recordEvent).toHaveBeenCalledWith('evt_1', 'stripe');
  });
