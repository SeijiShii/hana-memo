...
describe("buildConsentRecords", () => {
  ...
  it("UT-LE-A02: 3 件一括 → 3 レコード", () => {
    const recs = buildConsentRecords("user_1", [
      "privacy_policy",
      "terms_of_service",
      "ai_usage",
    ]);
    expect(recs).toHaveLength(3);
    expect(recs.map((r) => r.docType)).toEqual([
      "privacy_policy",
      "terms_of_service",
      "ai_usage",
    ]);
  ...
  it("UT-LE-A06: ip_hash を透過", () => {
    const recs = buildConsentRecords(
      "user_1",
      ["privacy_policy"],
      undefined,
      "deadbeef",
  ...
  it("userId 空 → ConsentError", () => {
    expect(() => buildConsentRecords("", ["privacy_policy"])).toThrow(
      ConsentError,
    );
  });

  it("UT-LE-E03: version null (cookie_policy) → ConsentError", () => {
    expect(() => buildConsentRecords("user_1", ["cookie_policy"])).toThrow(
      ConsentError,
    );
  });
});
...
describe("validateConsentInput", () => {
  it("正常 → throw しない", () => {
    expect(() =>
      validateConsentInput("privacy_policy", "v1.0.0"),
    ).not.toThrow();
  });
  it("null version → ConsentError", () => {
    expect(() => validateConsentInput("privacy_policy", null)).toThrow(
      ConsentError,
    );
  });
  it("形式不正 → ConsentError", () => {
    expect(() => validateConsentInput("privacy_policy", "1.0")).toThrow(
      ConsentError,
    );
  });
});
