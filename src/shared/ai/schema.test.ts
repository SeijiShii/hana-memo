...
const valid = {
  ...
  confidence: 0.82,
  similar_species: [
    { common_name: "ブタナ", reason_for_likely: "花が似ている" },
  ],
};
...
describe("parseIdentifyResponse", () => {
  ...
  it("正常 JSON string もパース", () => {
    expect(parseIdentifyResponse(JSON.stringify(valid)).commonName).toBe(
      "タンポポ",
    );
  });

  it("UT-AI-S02: 必須フィールド欠落 → SchemaValidationError", () => {
    const { scientific_name, ...rest } = valid;
    expect(() => parseIdentifyResponse(rest)).toThrow(SchemaValidationError);
  });

  it("UT-AI-S03: confidence < 0.6 → pending", () => {
    expect(parseIdentifyResponse({ ...valid, confidence: 0.4 }).status).toBe(
      "pending",
    );
  });

  it("UT-AI-S04: confidence >= 0.6 → identified", () => {
    expect(parseIdentifyResponse({ ...valid, confidence: 0.6 }).status).toBe(
      "identified",
    );
  });

  it("UT-AI-S05: confidence = 0 → unknown", () => {
    expect(parseIdentifyResponse({ ...valid, confidence: 0 }).status).toBe(
      "unknown",
    );
  });
  ...
  it("UT-AI-S06: similar_species > 3 → SchemaValidationError", () => {
    ...
    expect(() =>
      parseIdentifyResponse({ ...valid, similar_species: many }),
  ...
  it("不正 JSON string → SchemaValidationError", () => {
    expect(() => parseIdentifyResponse("{not json")).toThrow(
      SchemaValidationError,
    );
  });
  ...
  it("similar_species の要素が非オブジェクト → SchemaValidationError", () => {
    expect(() =>
      parseIdentifyResponse({ ...valid, similar_species: ["not-object"] }),
    ).toThrow(SchemaValidationError);
  });
});
