...
## Phase 3 デプロイ (2026-05-27、/flow:release --deploy-only、function 統合完了後)
- **前提**: revise_001 function-consolidation 完了で api/ = 11 関数 (≤12)。
- **デプロイ**: `vercel deploy` (preview, inline --env/--build-env で test モード secret 24 var) → **成功** (exit 0, readyState READY)。**12-fn 上限を突破** (前回 blocker 解消)。
- **URL**: https://hana-memo-y1xda75gc-quadiishii-9506s-projects.vercel.app
- **post-deploy smoke**: curl は **401 "Authentication Required"** = Vercel **Deployment Protection** (Hobby preview 既定の SSO 壁)。アプリ起因でなく Vercel 設定。→ curl 不可、ブラウザ (Vercel ログイン済オーナー) でアクセスして確認する必要あり。
- **次**: (1) ユーザーがブラウザで preview URL を開き boot/guest/撮影→識別/Checkout を目視、(2) または Vercel project の Deployment Protection を無効化して公開確認、(3) prod 化は `vercel deploy --prod` + Clerk webhook URL→`/api/auth/clerk-webhook` + R2 CORS に本番 origin + live 課金判断。

```yaml
- id: D20260527-001
  timestamp: 2026-05-27T00:30:00+09:00
  command: /flow:release
  phase: Phase3 deploy (--deploy-only)
  question: preview デプロイ実行 (Class B)
  chosen: 実行 → 成功 (11 fn で上限クリア、URL READY)
  chosen_type: explicit-choice
  context: ユーザー承認。inline test-mode env。post-deploy smoke は Vercel Deployment Protection (401 SSO) でブラウザ必須。
```
